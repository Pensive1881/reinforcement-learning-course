...
def update_Q(states_lst, actions_lst, rewards_lst, gamma, Q_accumulated, Q_visit_counter):
    '''
    '''
    # converting lists to numpy array for ease of use
    states_lst = np.array(states_lst)
    actions_lst = np.array(actions_lst)
    rewards_lst = np.array(rewards_lst)
    
    # finding the first time
    n, _, _ = np.shape(Q_accumulated)
